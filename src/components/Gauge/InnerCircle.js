...
import { projectionHeaderMessage } from "../../utils/utils";
      ...
      {payload.name === "Not Expected" && (
        ...
        >
          {`Every year during ${type.slice(
            11,
            type.lenght
          )} will have more than ${daysAbove} ${
            daysAbove === 1 ? "day" : "days"
          } > ${payload.t}˚F`}
        </text>
      )}
      ...
      {type !== "Observed Data" &&
        payload.name !== "Not Expected" && (
          <text
            ...
            fontSize={13}
          >
            {`This year's ${daysAbove} ${
              daysAbove === 1 ? "day" : "days"
            } would be considered ${projectionHeaderMessage(
              payload.name
